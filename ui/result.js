class Result {
  ...
  _renderWin(ctx, elapsed) {
    ...
    if (window.drawCyberpunkBtn) {
      drawCyberpunkBtn(ctx, r.x, r.y, r.w, r.h, '다시 도전', 'primary')
      drawCyberpunkBtn(ctx, m.x, m.y, m.w, m.h, '메뉴', 'dim')
      drawCyberpunkBtn(ctx, u.x, u.y, u.w, u.h, '업그레이드', 'blue')
    } else {
      ...
      ctx.fillStyle = '#ccaaff'; ctx.fillText('[ 메뉴 ]', m.x + m.w / 2, m.y + 28)
      ctx.fillStyle = '#88ff88'; ctx.fillText('[ 업그레이드 ]', u.x + u.w / 2, u.y + 28)
    }
    ctx.restore()
  ...
  _drawLosePanel(ctx, alpha, panelElapsed = 0) {
    ...
    if (window.drawCyberpunkBtn) {
      drawCyberpunkBtn(ctx, r.x, r.y, r.w, r.h, '재도전', 'blue')
      drawCyberpunkBtn(ctx, m.x, m.y, m.w, m.h, '메뉴', 'dim')
      drawCyberpunkBtn(ctx, u.x, u.y, u.w, u.h, '업그레이드', 'primary')
    } else {
